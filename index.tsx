...
const Navbar = () => (
  <nav className="sticky top-0 z-50 bg-[#FDFBF7]/95 backdrop-blur-md border-b-2 border-[#8C6A45]/30 px-6 py-4">
    <div className="max-w-6xl mx-auto flex justify-between items-center">
      <div className="flex items-center space-x-3">
        <div className="w-10 h-10 bg-[#B22222] rounded-full flex items-center justify-center text-white font-serif font-bold text-xl">醫</div>
      ...
      <div className="hidden md:flex space-x-8 text-sm font-bold text-[#8C6A45]">
        <a href="#rules" className="hover:text-[#B22222] transition-colors flex items-center gap-1"><BookOpen className="w-4 h-4" /> 遊戲玩法</a>
        <a href="#components" className="hover:text-[#B22222] transition-colors flex items-center gap-1"><Package className="w-4 h-4" /> 配件介紹</a>
        <a href="#oracle" className="hover:text-[#B22222] transition-colors flex items-center gap-1"><ScrollText className="w-4 h-4" /> 求籤互動</a>
        <a href="#knowledge" className="hover:text-[#B22222] transition-colors flex items-center gap-1"><Leaf className="w-4 h-4" /> 藥材知識</a>
      </div>
    </div>
  </nav>
);
...
const OracleSection = () => {
  ...
  const drawOracle = async () => {
    ...
    try {
      ...
      console.error(error);
      setOracleResult("靈藥尚未炮製完成，大道公指示暫待片刻。");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section id="oracle" className="py-24 px-6 bg-[#4A5D4E] relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5 pointer-events-none flex flex-wrap gap-20 justify-around items-center">
         {[...Array(6)].map((_, i) => <div key={i} className="text-9xl font-serif text-white">醫</div>)}
      </div>
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        <div className="bg-[#FDFBF7] p-8 md:p-12 rounded-sm shadow-2xl text-[#2D3A30] min-h-[450px] flex flex-col items-center justify-center border-[12px] border-double border-[#8C6A45]">
          {loading ? (
            <div className="flex flex-col items-center space-y-6">
              <div className="w-16 h-16 border-4 border-[#B22222] border-t-transparent rounded-full animate-spin"></div>
              <p className="text-[#8C6A45] font-serif font-bold animate-pulse">正在搖動古木籤筒...</p>
            </div>
          ) : oracleResult ? (
            <div className="animate-fadeIn w-full">
              <div className="mb-4 text-center">
                 <div className="inline-block px-4 py-1 border-2 border-[#B22222] text-[#B22222] font-bold text-lg mb-4">
                   保生大帝靈籤
                 </div>
              </div>
              <div className="whitespace-pre-wrap leading-relaxed text-[#2D3A30] max-w-lg mx-auto font-serif text-lg border-x border-[#8C6A45]/20 px-8 py-4">
                {oracleResult}
              </div>
              <button 
  ...
  return (
    <div className="min-h-screen text-[#2D3A30] selection:bg-[#B22222] selection:text-white">
      ...
      <header className="relative py-24 md:py-40 px-6 overflow-hidden bg-[#FDFBF7]">
        {/* Subtle scroll-like background texture decoration */}
        <div className="absolute top-0 left-0 w-full h-2 bg-[url('https://www.transparenttextures.com/patterns/handmade-paper.png')] opacity-20"></div>
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          <h1 className="text-7xl md:text-[10rem] font-bold mb-4 text-[#2D3A30] font-serif leading-tight">
             解藥籤 <span className="text-[#B22222] block md:inline">尋藥香</span>
          </h1>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <button className="px-10 py-4 bg-[#2D3A30] text-white font-serif tracking-widest text-lg hover:bg-black transition-all shadow-xl border-b-4 border-gray-900">
               探索醫道傳奇
            </button>
            <button className="px-10 py-4 border-2 border-[#8C6A45] text-[#8C6A45] font-serif tracking-widest text-lg hover:bg-[#8C6A45] hover:text-white transition-all">
               查看典藏版
            </button>
          </div>
        </div>
      ...
      <section id="rules" className="py-24 px-6 bg-white relative">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center mb-20 text-center">
            ...
            <p className="text-[#8C6A45] font-serif">Master the Ancient Healing Arts</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {[
              ...
              <div key={idx} className="relative p-10 bg-[#FDFBF7] border border-[#8C6A45]/20 group hover:border-[#B22222] transition-all duration-500">
                <div className="absolute -top-6 -left-6 w-12 h-12 bg-[#B22222] text-white flex items-center justify-center font-serif text-2xl font-bold shadow-lg">
                  {idx + 1}
                </div>
      ...
      {/* Knowledge Section - Manual Style */}
      <section id="knowledge" className="py-24 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-20">
          ...
          <h2 className="text-5xl font-bold font-serif">藥材百科</h2>
        </div>

        <div className="grid lg:grid-cols-3 gap-12">
          {HERBS.map((herb, idx) => (
            <div key={idx} className="group bg-[#FDFBF7] border-2 border-[#8C6A45]/20 hover:border-[#B22222] transition-all duration-700">
              <div className="relative h-80 overflow-hidden m-4 grayscale hover:grayscale-0 transition-all duration-700 border border-[#8C6A45]/10">
                <img 
                  src={herb.image} 
                  alt={herb.name} 
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                       ...
                       <span className="text-[10px] font-bold text-[#8C6A45] tracking-widest">傳統醫典記載</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="block text-[10px] text-[#8C6A45] mb-1 font-bold">藥性</span>
                    <span className="px-3 py-1 bg-[#8C6A45] text-white text-xs rounded-full font-serif font-bold">{herb.property}</span>
                  </div>
                </div>
                
                <p className="text-sm text-gray-600 mb-8 leading-relaxed font-serif italic border-l-2 border-[#8C6A45]/20 pl-4">
                  「{herb.description}」
                </p>
                
                <div className="space-y-3 bg-white p-4 border border-[#8C6A45]/5">
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-4 gap-16 relative z-10">
          <div className="col-span-2">
            ...
            <div className="flex space-x-6">
              <div className="w-12 h-12 border border-white/20 rounded-full flex items-center justify-center hover:bg-[#B22222] hover:border-[#B22222] transition-all cursor-pointer group">
                <span className="font-serif group-hover:scale-110 transition-transform">FB</span>
              </div>
              <div className="w-12 h-12 border border-white/20 rounded-full flex items-center justify-center hover:bg-[#B22222] hover:border-[#B22222] transition-all cursor-pointer group">
                <span className="font-serif group-hover:scale-110 transition-transform">IG</span>
              </div>
              <div className="w-12 h-12 border border-white/20 rounded-full flex items-center justify-center hover:bg-[#B22222] hover:border-[#B22222] transition-all cursor-pointer group">
                <span className="font-serif group-hover:scale-110 transition-transform">YT</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 opacity-70 font-serif">
              <li><a href="#" className="hover:text-[#B22222] transition-colors">宮廟足跡</a></li>
              <li><a href="#" className="hover:text-[#B22222] transition-colors">草本手稿</a></li>
              <li><a href="#" className="hover:text-[#B22222] transition-colors">募資進度</a></li>
              <li><a href="#" className="hover:text-[#B22222] transition-colors">聯繫我們</a></li>
            </ul>
          </div>
          
          <div>
            <h4 className="font-bold mb-8 text-[#B22222] uppercase tracking-[0.3em] font-serif border-b border-[#B22222]/30 pb-2">訂閱情報單</h4>
            <p className="text-xs opacity-50 mb-4 font-serif">第一時間獲取大道公的最新贈藥訊息</p>
            <div className="flex flex-col space-y-3">
              <input type="email" placeholder="您的電子信箱" className="bg-white/5 border border-white/10 px-4 py-3 w-full focus:ring-1 focus:ring-[#B22222] outline-none transition-all font-serif" />
              <button className="bg-[#B22222] px-6 py-3 font-serif font-bold hover:bg-red-800 transition-colors flex items-center justify-center gap-2">
                 確認訂閱 <ChevronRight className="w-4 h-4" />
              </button>
            </div>
          </div>
        </div>
